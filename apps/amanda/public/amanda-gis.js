...
(function (amanda) {
    ...
    var AmandaApp = (function () {
        ...
        AmandaApp.prototype.initialize = function (map, options) {
            ...
            this.trace.log("Initializing version {0} of the AMANDA GIS Widget".format(amanda.version), amanda.diagnostics.LogLevel.debug);
            ...
            if (this.commands.messageHandlerRegistered) {
                this.trace.log("Command handler registered", amanda.diagnostics.LogLevel.debug);
            }
            else {
                this.trace.log("Unable to attach command handler to window", amanda.diagnostics.LogLevel.error);
            }
            ...
            if (map && map instanceof esri.Map) {
                ...
                // map not recognized
                this.trace.log("The map used during initialization was not of recognized type. Currently Esri maps are supported", amanda.diagnostics.LogLevel.error);
            }
    ...
    (function (commands) {
        ...
        var Command = (function () {
            ...
            Command.prototype.execute = function (payload) {
                this.app.trace.log("Executing command '{0}'".format(this.name), amanda.diagnostics.LogLevel.debug);
    ...
    (function (commands) {
        ...
        var EsriCommandImpl = (function () {
            ...
            EsriCommandImpl.prototype.registerCommands = function () {
                this.app.commands.registerCommand(commands.CommandType.select, this, this.select);
                this.app.commands.registerCommand(commands.CommandType.activate, this, this.activate);
                this.app.commands.registerCommand(commands.CommandType.bufferSearch, this, this.bufferSearch);
                this.app.commands.registerCommand(commands.CommandType.deactivate, this, this.deactivate);
                this.app.commands.registerCommand(commands.CommandType.deselect, this, this.deselect);
                this.app.commands.registerCommand(commands.CommandType.highlight, this, this.highlight);
                this.app.commands.registerCommand(commands.CommandType.setLayerVisibility, this, this.setLayerVisibility);
                this.app.commands.registerCommand(commands.CommandType.setMapMode, this, this.setMapMode);
                this.app.commands.registerCommand(commands.CommandType.unhighlight, this, this.unhighlight);
                this.app.commands.registerCommand(commands.CommandType.setScale, this, this.setScale);
            };
            /**
            ...
            * {@link Payload} required command parameters:
            ...
            EsriCommandImpl.prototype.setScale = function (payload) {
                ...
                if (!scale) {
                    this.app.trace.log("Invalid payload for the scale command: Missing a valid scale..", amanda.diagnostics.LogLevel.error);
                    return;
                }
            ...
            EsriCommandImpl.prototype.select = function (payload) {
                ...
                if (!payload || !layerName || !fieldName) {
                    this.app.trace.log("Invalid payload for the select command. Missing layerName and/or fieldName.", amanda.diagnostics.LogLevel.error);
                    return;
                }
                var layer = this.app.map.getLayerByName(layerName);
                if (!layer) {
                    this.app.trace.log("Can not find a layer for {0}.".format(layerName), amanda.diagnostics.LogLevel.error);
                    return;
                }
                if (layer) {
                    var whereClause = "";
                    ...
                    var whereAuto = null;
                    if (payload && payload.fieldName && payload.features && payload.features.length > 0) {
                        whereAuto = amanda.utils.QueryHelper.generateWhereClause(payload.fieldName, payload.features, amanda.utils.QueryHelper.isFieldStringType(layer, fieldName));
                    }
                    var selectQuery = new amanda.map.Query(layer.getUrl());
                    selectQuery.queryParams.outFields.push("*");
                    selectQuery.queryParams.outSpatialReference = this.app.map.getGISMap().spatialReference;
                    ...
                    selectQuery.perform().then(function (results) {
                        var features = [];
                        if (results && results.features) {
                            ...
                            _this.app.events.featuresSelected(layerName, features, results.exceededTransferLimit);
                            _this._selectFeatures(layerName, fieldName, results.features);
                        }
                        else {
                            _this.app.trace.log("Select query returned no features", amanda.diagnostics.LogLevel.warning);
                        }
            ...
            EsriCommandImpl.prototype.setMapMode = function (payload) {
                ...
                var fieldName = payload.fieldName;
                if (selectionType.toUpperCase() === amanda.map.SelectionType.defaultSelect) {
                ...
                if (layer && fieldName && selectionType) {
                    ...
                    var geometrySelected = function (geometry) {
                        ...
                        selectQuery.queryParams.outFields.push("*");
                        selectQuery.queryParams.outSpatialReference = _this.app.map.getGISMap().spatialReference;
                        selectQuery.perform().then(function (results) {
                            var ids = [];
                            var attributes = [];
                            if (results && results.features) {
                                results.features.forEach(function (feature) {
                                    var id = _this._getPropertyByName(feature.attributes, fieldName);
                                    if (id) {
                                        ids.push(id);
                                    }
                                ...
                                if (results.exceededTransferLimit) {
                                    _this.app.trace.log("Select by geometry returned the first {0} results. Server limit exceeded.", amanda.diagnostics.LogLevel.warning);
                                }
                                _this.app.events.featuresSelectedByGeometry(layerName, attributes, ids, results.exceededTransferLimit);
                            }
                            else {
                                _this.app.trace.log("Select by geometry returned no features", amanda.diagnostics.LogLevel.warning);
                            }
                        }).catch(function (error) { return _this.app.trace.logError("Select by geometry query failed", error); });
                    };
                    this.app.map.captureSelection(selectionType, geometrySelected, { bufferUnit: payload.bufferUnit, bufferDistance: payload.distance });
                }
            };
            /**
            ...
            EsriCommandImpl.prototype.activate = function (payload) {
                ...
                var fieldName = payload.fieldName;
                var layer = this.app.map.getLayerByName(layerName);
                var stringType = amanda.utils.QueryHelper.isFieldStringType(layer, fieldName);
                var selectQuery = new amanda.map.Query(layer.getUrl());
                selectQuery.queryParams.outFields.push("*");
                selectQuery.queryParams.outSpatialReference = this.app.map.getGISMap().spatialReference;
                selectQuery.queryParams.where = amanda.utils.QueryHelper.generateWhereClause(fieldName, [feature], stringType);
                selectQuery.perform().then(function (results) {
                    if (results && results.features) {
                        results.features.forEach(function (graphic) {
                            var point = amanda.utils.GraphicHelper.getCentroid(graphic);
                            if (point) {
                                var pushpin = amanda.utils.GraphicHelper.createPushpinGraphic(point, _this.app.configOptions.pushpinImage);
                                pushpin.attributes = graphic.attributes;
                                var id = _this._getPropertyByName(graphic.attributes, fieldName);
                                amanda.utils.GraphicHelper.setTags(pushpin, layerName, commands.CommandType.activate, id);
                        ...
                        _this.app.trace.log("Activate query returned no features", amanda.diagnostics.LogLevel.warning);
                    }
                }).catch(function (error) {
                ...
                selectQuery.perform().then(function (results) {
                    if (results && results.features) {
                        results.features.forEach(function (highlightGraphic) {
                            if (highlightGraphic) {
                                var id = _this._getPropertyByName(highlightGraphic.attributes, fieldName);
                                amanda.utils.GraphicHelper.setTags(highlightGraphic, layerName, commands.CommandType.highlight, id);
                        ...
                        _this.app.trace.log("Highlight query returned no features", amanda.diagnostics.LogLevel.warning);
                    }
                }).catch(function (error) {
            ...
            EsriCommandImpl.prototype.bufferSearch = function (payload) {
                ...
                var stringType = amanda.utils.QueryHelper.isFieldStringType(sourceLayer, sourceFieldName);
                var selectQuery = new amanda.map.Query(sourceLayer.getUrl());
                selectQuery.queryParams.outFields.push("*");
                selectQuery.queryParams.outSpatialReference = this.app.map.getGISMap().spatialReference;
                selectQuery.queryParams.where = amanda.utils.QueryHelper.generateWhereClause(sourceFieldName, features, stringType);
                selectQuery.perform()
                    .then(function (results) {
                    var buffer = new amanda.map.Buffer(_this.app.map.geometryServiceUrl);
                    if (results && results.features && results.features.length > 0) {
                        results.features.forEach(function (toBuffer) {
                            if (toBuffer) {
                                ...
                                buffer.bufferParams.unit = amanda.map.Buffer.toEsriUnit(payload.bufferUnit);
                            }
                        });
                        return buffer.perform();
                    }
                    else {
                        _this.app.trace.log("Source layer buffer search query returned no features", amanda.diagnostics.LogLevel.warning);
                        return Promise.resolve([]);
                    }
                }).then(function (bufferResults) {
                    if (bufferResults && bufferResults.length > 0) {
                        var geometryToQueryWith = bufferResults[0];
                        var stringType_1 = amanda.utils.QueryHelper.isFieldStringType(layer, fieldName);
                        var selectQuery_1 = new amanda.map.Query(layer.getUrl());
                        selectQuery_1.queryParams.outFields.push("*");
                        selectQuery_1.queryParams.outSpatialReference = _this.app.map.getGISMap().spatialReference;
                        selectQuery_1.queryParams.geometry = geometryToQueryWith;
                        return selectQuery_1.perform();
                    }
                    else {
                        _this.app.trace.log("No buffer could be determined from source input features", amanda.diagnostics.LogLevel.warning);
                        return Promise.resolve(new esri.tasks.FeatureSet());
                    }
                }).then(function (foundFeatures) {
                    if (foundFeatures && foundFeatures.features && foundFeatures.features.length > 0) {
                        var idsArray_1 = [];
                        ...
                        _this._selectFeatures(layerName, fieldName, foundFeatures.features);
                    }
                    else {
                        _this.app.trace.log("Buffered query did not return any results", amanda.diagnostics.LogLevel.warning);
                    }
                    return Promise.resolve();
                }).catch(function (error) {
                    _this.app.trace.logError("Failed to perform the buffered selection", error);
                });
            };
            EsriCommandImpl.prototype._selectFeatures = function (layerName, fieldName, graphics) {
                var _this = this;
                this.app.map.ensurePushpinsOntop();
                graphics.forEach(function (graphic) {
                    var id = _this._getPropertyByName(graphic.attributes, fieldName);
                    amanda.utils.GraphicHelper.setTags(graphic, layerName, commands.CommandType.select, id);
                    _this.app.map.addPushpin(graphic);
                });
                this.app.map.zoomToSelection(null);
            };
            EsriCommandImpl.prototype._getPropertyByName = function (array, fieldName) {
                var val = null;
        ...
        var EventHandler = (function () {
            ...
            EventHandler.prototype.initialize = function (options) {
                if (options && !!options["domain"]) {
                    ...
                    this.app.trace.log("Event handler using domain: ".format(this._domain), amanda.diagnostics.LogLevel.debug);
                }
             ...
             */
            EventHandler.prototype.featuresSelected = function (layerName, features, exceededTransferLimit) {
                if (exceededTransferLimit === void 0) { exceededTransferLimit = false; }
                var payload = new events.Payload();
             ...
             * @param features The ids of the features.
             */
            EventHandler.prototype.featuresSelectedByGeometry = function (layerName, attributes, features, exceededTransferLimit) {
                if (exceededTransferLimit === void 0) { exceededTransferLimit = false; }
                var payload = new events.Payload();
             ...
             * @param features The ids of the features.
             */
            EventHandler.prototype.selectedFeaturesVisibleOnMap = function (features) {
                var payload = new events.Payload();
             ...
             * @param features The ids of the features.
             */
            EventHandler.prototype.bufferSearchResults = function (layerName, features) {
                var payload = new events.Payload();
             ...
             */
            EventHandler.prototype.visibilityChanged = function (layerName, visibility) {
                var payload = new events.Payload();
            ...
            EventHandler.prototype.emitEvent = function (event) {
                if (!(event instanceof events.Payload)) {
                    this.app.trace.log("Event must be of type amanda.events.Payload.", amanda.diagnostics.LogLevel.error);
                    return;
                }
                var windowDomain = "*";
                if (this._domain) {
                ...
                try {
                    ...
                    this.app.trace.log("'{0}' was emitted from the map.".format(event.messageType), amanda.diagnostics.LogLevel.debug);
                }
                catch (e) {
...
(function (amanda) {
    ...
    var EsriLayer = (function () {
        ...
        EsriLayer.prototype.setVisibility = function (visibility) {
            ...
            else if (this._serviceLayer instanceof esri.layers.ArcGISDynamicMapServiceLayer) {
        ...
        EsriLayer.prototype._populateQueryInfo = function () {
            ...
            infoQuery.perform().then(function (results) {
    ...
    (function (map_1) {
        ...
        var EsriMap = (function () {
            ...
            function EsriMap(app, map, options) {
                ...
                this._defaultGeometryServiceUrl = "http://sampleserver1.arcgisonline.com/ArcGIS/rest/services/Geometry/GeometryServer";
                ...
                this._mapInitialized = function () {
                    ...
                    _this._esriMap.on("extent-change", function (extentChange) { return _this._handleExtentChanged(extentChange); });
                    // attach layer-add handler
                    _this._esriMap.on("layer-add", function (args) { return _this._handleLayerAdded(args); });
                    // add pushpin layer event listeners
                    _this._pushpinLayer.on("click", function (event) { return _this._handlePushpinClick(event); });
                    ...
                    _this._pushpinLayer.on("mouse-over", function (event) {
                        if (event && event.graphic) {
                            ...
                            event.graphic.setSymbol(amanda.utils.GraphicHelper.getHighlightedSymbol(_this.app.configOptions.highlightedPushpinImage));
                    ...
                    _this._pushpinLayer.on("mouse-out", function (event) {
                        if (event && event.graphic) {
                            var tags = amanda.utils.GraphicHelper.extractTags(event.graphic);
                            event.graphic.setSymbol(amanda.utils.GraphicHelper.getSelectedSymbol(_this.app.configOptions.pushpinImage));
                        }
                    });
                    // attach highlight click handler
                    _this._highlightLayer.on("click", function (event) { return _this._handlePushpinClick(event); });
                ...
                if (!this.geometryServiceUrl) {
                    ...
                    this.app.trace.log("Using default geometry service: '{0}'".format(this._defaultGeometryServiceUrl), amanda.diagnostics.LogLevel.debug);
                }
                var paramToken = amanda.utils.Extensions.getParameterByName("token");
                if (paramToken) {
                    this._serviceToken = paramToken;
                    this.app.trace.log("Service token loaded from parent window URL", amanda.diagnostics.LogLevel.debug);
                }
                this._initialize();
            }
                ...
                if (this._esriMap.loaded) {
                    ...
                    if (this._esriMap && this._esriMap.graphicsLayerIds) {
                        for (var i = 0; i < this._esriMap.graphicsLayerIds.length; i++) {
                            ...
                            args.layer = this._esriMap.getLayer(this._esriMap.graphicsLayerIds[i]);
            ...
            EsriMap.prototype._parseOptions = function (options) {
                if (options) {
                    if (!!options["proxyUrl"]) {
                        ...
                        this.app.trace.log("Proxy URL set to: {0}".format(url), amanda.diagnostics.LogLevel.debug);
                    }
                    ...
                    if (!!options["serviceToken"] && !this._serviceToken) {
                        ...
                        this.app.trace.log("Service token loaded from configuration", amanda.diagnostics.LogLevel.debug);
                    }
                    if (!!options["serviceUrl"]) {
                        this._serviceUrl = options["serviceUrl"];
                        this.app.trace.log("Service URL loaded from configuration", amanda.diagnostics.LogLevel.debug);
                    }
                    if (!!options["expandRatio"]) {
                        this._expandRatio = options["expandRatio"];
                        this.app.trace.log("Expand ration loaded from configuration", amanda.diagnostics.LogLevel.debug);
                    }
                    if (!!options["geometryServiceUrl"]) {
                        this.geometryServiceUrl = options.geometryServiceUrl;
                        this.app.trace.log("Geometry Service Url set to {0} from configuration".format(this.geometryServiceUrl), amanda.diagnostics.LogLevel.debug);
                    }
                }
                else {
                    this.app.trace.log("Service URL was not specified. CSDC Feature Service tokens cannot be replaced.", amanda.diagnostics.LogLevel.warning);
                }
            };
            /**
            ...
            EsriMap.prototype.setExtent = function (extent) {
                if (extent) {
                    ...
                    this.app.trace.log("Setting extent with expand ratio of {0}".format(this._expandRatio), amanda.diagnostics.LogLevel.debug);
                }
            };
            ...
            EsriMap.prototype._handleAddLayer = function (amandaLayer, layer) {
                if (amandaLayer) {
                    ...
                    if (this._serviceToken && this._serviceUrl && layer && layer._url && layer.url.toLowerCase().indexOf(this._serviceUrl.toLowerCase()) > -1 && layer.url.toLowerCase().indexOf("token") > -1) {
                        ...
                        if (_url.query) {
                            ...
                            this.app.trace.log("Replacing AMANDA Cloud URL token on layer: '{0}'".format(amandaLayer.name), amanda.diagnostics.LogLevel.debug);
                        }
                        if (layer instanceof esri.layers.FeatureLayer) {
                            ...
                            if (_task && _task._url && _task._url.query) {
                                ...
                                this.app.trace.log("Replacing AMANDA Cloud token on FeatureService task on layer: '{0}'".format(amandaLayer.name), amanda.diagnostics.LogLevel.debug);
                            }
                        }
                    }
                    this.layers.push(amandaLayer);
                    this.app.trace.log("Layer '{0}' was added and will be available to participate in AMANDA GIS operations.".format(amandaLayer.name), amanda.diagnostics.LogLevel.debug);
                }
            };
                    ...
                    if (this._mapHasActivePushpins()) {
                        ...
                        for (var i = 0; i < pushpins.length; i++) {
                            ...
                            if (extent.contains(point)) {
                                ...
                                var insertIndex = this._getVisibleResultsLayerIndex(tags.layerName, visibleFeatures);
            ...
            EsriMap.prototype._getVisibleResultsLayerIndex = function (layerName, visibleFeatures) {
                var existsAtIndex = -1;
                if (layerName && visibleFeatures && visibleFeatures.length) {
                    for (var i = 0; i < visibleFeatures.length; i++) {
                        ...
                        if (visLayerName && visLayerName.toLowerCase() === layerName.toLowerCase()) {
            ...
            EsriMap.prototype.getLayerByName = function (name) {
                ...
                if (!name) {
                    ...
                    for (var i = 0; i < this.layers.length; i++) {
                        if (name.toLowerCase().trim() === this.layers[i].name.toLowerCase().trim()) {
                            //found the layer
                            layer = this.layers[i];
                            break;
                        }
                ...
                // none found
                if (!layer) {
                    this.app.trace.log("Could not find a layer with name: '{0}'. Cannot execute command.".format(name), amanda.diagnostics.LogLevel.error);
                }
                return layer;
            };
            /**
            ...
            EsriMap.prototype.addPushpin = function (graphic) {
                if (graphic && graphic.geometry) {
                    ...
                    graphic.setSymbol(amanda.utils.GraphicHelper.getSelectedSymbol(this.app.configOptions.pushpinImage));
            ...
            EsriMap.prototype.addHighlight = function (graphic) {
                if (graphic && graphic.geometry) {
                    var newGeometry = amanda.utils.GraphicHelper.getCentroid(graphic);
                    graphic.setGeometry(newGeometry);
                    graphic.setSymbol(amanda.utils.GraphicHelper.getHighlightedSymbol(this.app.configOptions.highlightedPushpinImage));
            ...
            EsriMap.prototype._activateSelection = function (esriMode) {
                this.app.trace.log("Activating draw toolbar: {0}".format(esriMode), amanda.diagnostics.LogLevel.debug);
                this._drawingTool.activate(esriMode);
                this._drawingActive = true;
            };
            EsriMap.prototype._deactivateSelection = function () {
                this.app.trace.log("Deactivating draw toolbar", amanda.diagnostics.LogLevel.debug);
            ...
            EsriMap.prototype.ensurePushpinsOntop = function () {
                if (this._esriMap.graphicsLayerIds) {
                    ...
                    var pushpinIx = this._esriMap.graphicsLayerIds.indexOf(this._pushpinLayer.id);
                    var hpushpinIx = this._esriMap.graphicsLayerIds.indexOf(this._highlightLayer.id);
             ...
             */
            EsriMap.prototype.captureSelection = function (mode, selectedCallback, options) {
                var _this = this;
                var draw = true;
                if (this._drawingActive) {
                    this.app.trace.log("Selection already active, deactivating previous selection", amanda.diagnostics.LogLevel.debug);
                    this._deactivateSelection();
                }
                if (mode) {
                    this.app.events.mapModeChanged(mode);
                    this.app.trace.log("'{0}' selection mode received".format(mode), amanda.diagnostics.LogLevel.debug);
                    ...
                    if (draw) {
                        ...
                        this._drawEventHandle = this._drawingTool.on("draw-complete", function (drawEvent) {
                            _this.app.trace.log("Draw completed", amanda.diagnostics.LogLevel.debug);
                            _this._performMapModeBuffer(drawEvent.geometry, options.bufferUnit, options.bufferDistance).then(function (geometry) {
                                var payload = new amanda.events.Payload();
                                payload.messageType = amanda.events.EventType.geometrySelectionCaptured;
                                payload.mapMode = mode.toLowerCase();
                                _this.app.events.emitEvent(payload);
                                selectedCallback(geometry);
                            }).catch(function (error) {
                                _this.app.trace.logError("Failed to buffer the map selection point.", error);
                            });
                        });
                    ...
                    this.app.trace.log("Selection mode was null or undefined", amanda.diagnostics.LogLevel.error);
                }
            };
            EsriMap.prototype.zoomToSelection = function (extent) {
                var toZoom = extent ? extent : esri.graphicsExtent(this._pushpinLayer.graphics).expand(1.6);
                this._esriMap.setExtent(toZoom);
            };
            EsriMap.prototype._performMapModeBuffer = function (geometry, bufferUnit, bufferDistance) {
                var _this = this;
                return new Promise(function (resolve, reject) {
                    if (!bufferUnit || !bufferDistance) {
                        _this.app.trace.log("Selection will not be buffered, no unit or distance specified.", amanda.diagnostics.LogLevel.debug);
                        resolve(geometry);
                    }
                    else {
                        ...
                        bufferTask.bufferParams.outSpatialReference = _this._esriMap.spatialReference;
                        bufferTask.bufferParams.unit = map_1.Buffer.toEsriUnit(bufferUnit);
                        bufferTask.perform().then(function (bufferedGeometries) {
        ...
        var CommandHandler = (function () {
            ...
            CommandHandler.prototype.registerMessageHandler = function () {
                ...
                if (window.addEventListener) {
                    window.addEventListener('message', function (event) { return _this._handleAMANDACommand(event); }, false); // FF,SA,CH,OP,IE9+
                    this.app.trace.log("FF,SA,CH,OP,IE9+ message handler registered", amanda.diagnostics.LogLevel.debug);
                    this.messageHandlerRegistered = true;
                }
                else if (window.attachEvent) {
                    window.attachEvent('onmessage', function (event) { return _this._handleAMANDACommand(event); }); // IE8 although we do not officially support this in AMANDA 7
                    this.app.trace.log("IE8 message handler registered", amanda.diagnostics.LogLevel.debug);
                    this.messageHandlerRegistered = true;
                }
                else {
                    this.messageHandlerRegistered = false;
                    this.app.trace.log("Failed to register message handler, app cannot function", amanda.diagnostics.LogLevel.error);
                }
            };
            /**
             ...
             */
            CommandHandler.prototype.registerCommand = function (commandName, scope, func) {
                if (!commandName || !scope || !func) {
                    this.app.trace.log("'commandName', 'scope' and 'func' are required for creating commands".format(commandName), amanda.diagnostics.LogLevel.error);
                    return;
                }
                commandName = commandName.toLowerCase();
                if (this._commandExists(commandName)) {
                    this.app.trace.log("Overwriting the command '{0}'".format(commandName), amanda.diagnostics.LogLevel.warning);
                }
                var command = new commands.Command(this.app, commandName, scope, func);
                this.namedCommands[commandName] = command;
                this.app.trace.log("Command '{0}' added to the registry".format(commandName), amanda.diagnostics.LogLevel.debug);
            };
            /**
            ...
            CommandHandler.prototype.unregisterCommand = function (commandName) {
                if (commandName && this._commandExists(commandName)) {
                    ...
                    this.app.trace.log("Unregistered command '{0}'".format(commandName), amanda.diagnostics.LogLevel.debug);
                }
            };
            ...
            CommandHandler.prototype._handleAMANDACommand = function (browserEvent) {
                if (!browserEvent || !browserEvent.data) {
                    this.app.trace.log("Command or Command data not found", amanda.diagnostics.LogLevel.warning);
                    return;
                }
                var commandPayload = commands.Payload.fromJson(browserEvent.data);
                if (!commandPayload || !commandPayload.messageType) {
                    this.app.trace.log("Command received, but did not contain a messageType key. Cannot execute any command", amanda.diagnostics.LogLevel.error);
                    return;
                }
                else {
                    this.app.trace.log("Command '{0}' received from AMANDA".format(commandPayload.messageType), amanda.diagnostics.LogLevel.debug);
                }
                ...
                if (this.namedCommands[commandName]) {
                    ...
                    this.app.trace.log("Command '{0}' not handled".format(commandName), amanda.diagnostics.LogLevel.error);
                }
            };
    ...
    (function (utils) {
        ...
        var GraphicHelper = (function () {
            ...
            GraphicHelper.createPushpinGraphic = function (point, image) {
                return new esri.Graphic(point, GraphicHelper.getSelectedSymbol(image), null);
            };
            /**
            ...
            GraphicHelper.getHighlightedSymbol = function (imageUrl) {
                ...
                var defaultImageUrl = "./resources/images/pushpins/map-marker-blue-32.png";
                if (!imageUrl) {
            ...
            GraphicHelper.getSelectedSymbol = function (imageUrl) {
                if (imageUrl === void 0) { imageUrl = null; }
                var defaultImageUrl = "./resources/images/pushpins/map-marker-red-32.png";
                if (!imageUrl) {
                    imageUrl = defaultImageUrl;
                }
                return new esri.symbol.PictureMarkerSymbol({
             ...
             */
            GraphicHelper.setTags = function (graphic, layerName, commandName, gisId) {
                if (!graphic) {
                    return;
                }
                if (layerName) {
            ...
            QueryHelper.isFieldStringType = function (layer, fieldName) {
                ...
                if (layer && layer.getLayerFields() && fieldName) {
                    ...
                    for (var i = 0; i < fields.length; i++) {
                        var field = fields[i];
                        if (field.name.toUpperCase() === fieldName || field.alias.toUpperCase() === fieldName) {
...
(function (amanda) {
    ...
    (function (diagnostics) {
        ...
        var Logger = (function () {
             ...
             */
            function Logger(app) {
                this.logLevelMap = [{ level: diagnostics.LogLevel.debug, order: 1 }, { level: diagnostics.LogLevel.warning, order: 2 }, { level: diagnostics.LogLevel.error, order: 3 }];
            ...
            Logger.prototype.log = function (message, messageLogLevel) {
                if (this._getLogLevel(messageLogLevel) >= this._getLogLevel(this._logLevel)) {
                    if (messageLogLevel) {
                        this._writeLog("{0} {1}: [{2}] {3}".format(this._logStart, this.getDate(), messageLogLevel, message));
                    }
                    else {
                        this._writeLog(this._logStart + Date() + ": " + message);
                        this._writeLog("{0} {1}: {2})".format(this._logStart, this.getDate(), message));
                    }
                }
            };
            /**
            ...
            Logger.prototype.logError = function (message, error) {
                this._writeLog("{0} {1}: [{2}] {3} Reason: {4}".format(this._logStart, this.getDate(), diagnostics.LogLevel.error, message, error ? error.message : "No details available"));
            };
            Logger.prototype.getDate = function () {
                var date = new Date();
                return "{0}:{1}:{2}.{3}".format(this._padDigits(date.getHours(), 2), this._padDigits(date.getMinutes(), 2), this._padDigits(date.getSeconds(), 2), this._padDigits(date.getMilliseconds(), 3));
            };
            ...
            Logger.prototype._padDigits = function (num, totalDigits, left) {
                ...
                return left ? (padding.toString() + numString) : (numString + padding.toString());
            };
            /**
             * Initializes the object from an {@link InitializationOptions}.
             * @param options The {@link InitializationOptions}.
            ...
            Logger.prototype.initialize = function (options) {
                if (options && !!options["logLevel"]) {
                    ...
                    if (level === diagnostics.LogLevel.debug ||
                        level === diagnostics.LogLevel.warning ||
                        level === diagnostics.LogLevel.error) {
                        this._logLevel = level;
                        this.log("Log level set to {0} from configuration".format(level), diagnostics.LogLevel.debug);
                    }
                    else {
                        this.log("Specified log level of {0} was not recognized. Values must be {1}, {2} or {3}".format(level, diagnostics.LogLevel.debug, diagnostics.LogLevel.warning, diagnostics.LogLevel.error), diagnostics.LogLevel.warning);
                    }
                }
                this.initialized = true;
            };
